...
function trivSetupPage()
{
	...
	if(!isSinglePagePlayerAvail())
	{
		try{
			if( is.isMobile.any() )
			{
				getDisplayWindow().myTop.addEventListener("orientationchange", changeSize, false);
...
function addFocusEvents() {
	var targetElements = "div[class^=\"mejs-button\"] button,div[class^=\"mejs-\"] span,div[class^=\"mejs-\"] a,div[id^=\"image\"] a,div[id^=\"shape\"] a,textarea,input[type=\"text\"],input[type=\"password\"],select,div[id^=\"image\"] a img,div[id^=\"shape\"] a img,div[id^=\"image\"] a svg,div[id^=\"shape\"] a svg,div[id^=\"text\"] a + div,input[type=\"file\"],input[type=\"radio\"],input[type=\"checkbox\"],button,div[id^=\"textbutton\"] a,div[id^=\"image\"] a,div[id^=\"shape\"] a";
	...
	for(var i = 0; i < pageElements.length;i++){
		...
		if(pageElements[i].parentNode.tagName == "pattern")
			continue;
		pageElements[i].addEventListener('focus',addFocusIndicator);
...
function getAncestorFrameOrIFrame(win, tag, cnt)
{
	...
	cnt = cnt || 0;
	if (win.frameElement && win.frameElement.tagName.toLowerCase() == tag && win.frameElement.parentElement.id != 'trivTitleMgrFrameset')
		return win.frameElement;
	else if (win.parent && cnt < 10)
		return getAncestorFrameOrIFrame(win.parent, tag, ++cnt);
	return null;
}
